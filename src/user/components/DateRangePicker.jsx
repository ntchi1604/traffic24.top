...
export default function DateRangePicker({ value, onChange, id = 'drp' }) {
  ...
  const [fromVal, setFromVal] = useState(value?.from || '2026-06-01')
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
      <div style={{
        ...
        <svg viewBox="0 0 24 24" fill="none" width="15" height="15"
          style={{ color: 'var(--ud-text-3)', marginLeft: 5, flexShrink: 0 }}>
        ...
        {PRESETS.map(r => (
          <button
            ...
            style={{
              ...
              color: isPresetActive(r.key) ? '#1A7FFF' : 'var(--ud-text-2)',
              fontSize: 12.5, fontWeight: isPresetActive(r.key) ? 700 : 500,
              fontFamily: "'Inter',sans-serif",
              transition: 'all 0.18s', whiteSpace: 'nowrap',
            }}
            onMouseEnter={e => { if (!isPresetActive(r.key)) e.currentTarget.style.color = 'var(--ud-text)' }}
            onMouseLeave={e => { if (!isPresetActive(r.key)) e.currentTarget.style.color = 'var(--ud-text-2)' }}
          >{r.label}</button>
        ))}
        ...
        <button
          ...
          style={{
            ...
            color: showCustom ? '#F59E0B' : 'var(--ud-text-2)',
            fontSize: 12.5, fontWeight: showCustom ? 700 : 500,
            fontFamily: "'Inter',sans-serif",
            transition: 'all 0.18s', whiteSpace: 'nowrap',
            display: 'flex', alignItems: 'center', gap: 5,
          }}
          onMouseEnter={e => { if (!showCustom) e.currentTarget.style.color = 'var(--ud-text)' }}
          onMouseLeave={e => { if (!showCustom) e.currentTarget.style.color = 'var(--ud-text-2)' }}
        >
      ...
      {showCustom && (
        <div style={{
          ...
          <span style={{ fontSize: 11.5, color: 'var(--ud-text-3)', whiteSpace: 'nowrap' }}>Từ</span>
          <input type="date" id={`${id}-from`} value={fromVal} max={toVal || TODAY}
            onChange={e => setFromVal(e.target.value)} style={dateInput} />
          <span style={{ fontSize: 11.5, color: 'var(--ud-text-3)' }}>—</span>
          <input type="date" id={`${id}-to`} value={toVal} min={fromVal} max={TODAY}
            onChange={e => setToVal(e.target.value)} style={dateInput} />
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
            <button id={`${id}-apply`} onClick={applyCustom}
              disabled={!fromVal || !toVal || fromVal > toVal}
              style={{
                padding: '5px 12px', borderRadius: 8, cursor: 'pointer',
                background: (!fromVal || !toVal || fromVal > toVal) ? 'var(--ud-surface-2)' : '#F59E0B',
                border: 'none',
                color: (!fromVal || !toVal || fromVal > toVal) ? 'var(--ud-text-3)' : '#000',
            ...
            {fromVal && toVal && fromVal <= toVal && (
              <span style={{
                ...
                background: 'rgba(245,158,11,0.12)', padding: '3px 8px', borderRadius: 6,
                whiteSpace: 'nowrap',
              }}>
